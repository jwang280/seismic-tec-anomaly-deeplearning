c     Compile with: gfortran -g -fbacktrace -ffpe-trap=zero,overflow,underflow ProcessOneDayCorr30MinMeansRand.f -o ProcessOneDayCorr30MinMeansRand

      PROGRAM ProcessOneDayCorr30MinMeansRand
      implicit none
      character*4 numberstrng
      character*10 cdate,datestrng,cyear,cmonth,cday
      character*20 eqname
      integer date(3),lg(10),i,j,j2,j3,k,k2,fcount,jmax,jmean,
     1     nd(450)
      ...
      real distkm,xbar(450),mean30min(48,450)
      ...
      elemnts2(4)=cdate
      elemnts2(5)="."
      elemnts2(6)="-30minmeans."
      elemnts2(7)="corr"
      fileout=paste(elemnts2,7)
      ...
      do j=60,2880,60
         ...
         do j2=(j-59),j
          ...
          if (fileex)then
            ...
            do k=1,450
             read(10,*,err=101,end=102)lat(k),long(k),tec(j2,k)
             mean30min(j3,k)=mean30min(j3,k)+tec(j2,k)/60
            enddo
 101        CONTINUE
          else
            go to 103 ! CYCLE
 ...
 103     enddo
         meantec(k)=meantec(k)+mean30min(j3,k)/48
 102  enddo
...
c     calculate correlation matrix for this day
         do j3=1,48
            xbar(k)=xbar(k)+mean30min(j3,k)/48
            xwiggle(j3,k)=0.
         enddo
         write(12,*)"flag1a",k,j3
         do k2=1,450
     ...
     1           long(k2)).lt.250)) then
      ...
      do k=1,450
         ...
         do j3=1,48
            ...
            xbarwiggle(k)=xbarwiggle(k)+xwiggle(j3,k)/48
         enddo
      enddo
      write(12,*)"flag2"
      do k=1,450
         ...
         sig2(k)=0.
         do j3=1,48
            sig1(k)=sig1(k)+(mean30min(j3,k)-xbar(k))**2/47
            sig2(k)=sig2(k)+(xwiggle(j3,k)-xbarwiggle(k))**2/47
         enddo
         sig1(k)=sqrt(sig1(k))
         sig2(k)=sqrt(sig2(k))
      enddo
      ...
      do k=1,450
         corr(k)=0.0
         do j3=1,48
          if ((sig1(k)*sig2(k)).eq.0.) then
c             write(6,*) cdate,"sigzero",lat(k),long(k),sig1(k),sig2(k)
             corr(k)=1.0
          else
             corr(k)=corr(k)+(mean30min(j3,k)-xbar(k))*(xwiggle(j3,k)
     1           -xbarwiggle(k))/(47*sig1(k)*sig2(k))
          endif
...
c      write(6,*)(lg(i),i=1,n)
 ...
 21      format(40a1)
         ...
         enddo
      enddo
      write(paste,22)((elemat(j,i),j=1,lg(i)),i=1,n)
 22   format(256a1)
      return
      end
